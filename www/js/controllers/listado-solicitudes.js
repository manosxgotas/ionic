angular.module('donacion')
  .controller('ListadoSolicitudesController', function ($scope, SolicitudesService, CentrosDonacionService, GruposSanguineosService) {
    ...
    $scope.filtrarSolicitudes = function () {
      ...
      var gruposAux =  GruposSanguineosService.infoGrupoSanguineo().query({id:$scope.currentUser.grupoSanguineo.id},function(){
        angular.forEach(gruposAux,function (value,key) {
          if (value.id == $scope.currentUser.grupoSanguineo.id){
            angular.forEach(value.puedeDonarA,function(valor,clave){

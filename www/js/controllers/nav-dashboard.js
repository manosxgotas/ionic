angular.module('donacion')
  .controller('NavDashboardController', function ($http, $scope, $rootScope, $uibModal, CurrentUserService, EventosService, DonacionesService, SolicitudesService, localStorageService) {
    ...
    $rootScope.navbar_initialized = false;
    ...
    lbd = {
      ...
      initRightMenu: function () {
        if (!$rootScope.navbar_initialized) {
          $navbar = $('nav').find('.navbar-collapse').first().clone(true);
          $navbar.attr('id', 'rightMenu')
          ...
          content_buff_sections = $sidebar.find('.nav-sections').html();
          ul_content = ul_content + content_buff + content_buff_sections;
          ...
          $rootScope.navbar_initialized = true;
        }
        var rightMenu = $(".navbar-collapse-dashboard");
        rightMenu.on("click", "a", null, function (e) {
          $('html').removeClass('nav-open');
          lbd.misc.navbar_menu_visible = 0;
          $('#bodyClick').remove();
          setTimeout(function () {
